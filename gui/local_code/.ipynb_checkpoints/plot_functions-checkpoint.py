...
def plot_time_series(df: pd.DataFrame, date_col: str, y_col: str) -> None:
    """
    ...
    df_date = df.groupby([date_col]).agg({y_col: 'sum', date_col: 'count'}).\
    rename(columns={y_col: 'sum', date_col: 'count'}).reset_index()
...
def plot_features_relationship(df: pd.DataFrame, x_column: str, y_column: str) -> None:
    '''
    ...
    if df[x_column].nunique() > 100:
        print(f"{x_column} has too many unique values: {df[x_column].nunique()}")
        return
...
def scatter_plot(df: pd.DataFrame, x_col: str, y_col: str) -> None:
    """
...
def whisker_plot(df: pd.DataFrame, x_col: str, y_col: str) -> None:
    """
...
def plot_distribution(df: pd.DataFrame, column: str, y: str = '', bins: int = 30) -> None:
    """
...
def density_heatmap_plot(df: pd.DataFrame, x_col: str, y_col: str) -> None:
    """
